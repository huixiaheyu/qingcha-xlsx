...
function disableDownload() {
  downloadButton.href = '#';
  downloadButton.className = 'pointer-events-none rounded bg-gray-300 px-6 py-2 text-sm font-semibold text-white';
  downloadButton.setAttribute('aria-disabled', 'true');
}

function enableDownload(url) {
  downloadButton.href = url;
  downloadButton.className = 'rounded bg-emerald-600 px-6 py-2 text-sm font-semibold text-white shadow-sm hover:bg-emerald-700';
...
async function loadRequiredFiles() {
  ...
  data.requiredFiles.forEach((filename) => {
    ...
    item.className = 'rounded bg-white px-2 py-2 ring-1 ring-gray-200';
